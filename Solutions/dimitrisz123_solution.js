...
const minRecurse = (first, ...nums) => {
  ...
  return first < second
    ? minRecurse(first, ...rest)
    : minRecurse(second, ...rest);
};
...
const maxRecurse = (first, ...nums) => {
  ...
  return first > second
    ? maxRecurse(first, ...rest)
    : maxRecurse(second, ...rest);
};
    ...
    return {
      value:
        typeof args[0] === "number"
          ? f(...args)
          : f(...args.map((arg) => arg.value)),
      source:
        typeof args[0] === "number"
          ? `(${args.join(op)})`
          : `(${args.map((arg) => arg.source).join(op)})`,
    };
  };
